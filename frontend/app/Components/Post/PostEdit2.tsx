...
const PostEdit2 = () => {
  ...
  { icon: FlipHorizontal, label: "Flip", index: "2" },],[ { icon: Filter, label: "Filter", index: "3" },
  { icon: ZoomIn, label: "Zoom In", index: "4" },
  { icon: ZoomOut, label: "Zoom Out", index: "5" },
  { icon: Trash, label: "Delete", index: "6" },{ icon: Settings, label: "Settings", index: "7" },
]
    ...
    return (
        <div className="min-h-screen bg-blue-400 text-white flex flex-col justify-center items-center pt-[80px]">
            <div className="w-[500px] bg-zinc-300 h-[400px] border-2 border-white shadow-lg flex flex-col items-center gap-3">
                {/* header */}
                <div className="w-full h-8 bg-[#000080] flex justify-between p-1">
                    ...
                    <div className="w-[100px] flex gap-1 h-full  justify-end">
                        <button onClick={() => navigate(`/postDetail/${id}`)} className="h-full bg-gray-300 hover:border border-2 border-b-black border-r-black text-center grid place-items-center leading-none hover:bg-red-600">
                            <Minus className="text-black w-full h-full"></Minus>
                        </button>
                        <button  onClick={() => navigate(`/News`)} className="h-full bg-gray-300 hover:border border-2 border-b-black border-r-black text-center grid place-items-center leading-none hover:bg-red-600">
                ...
                <div className="w-full h-18 flex">
                    ...
                    <div className="w-1/2 h-full flex flex-col px-4">
                        {buttonLayout.map((baris, layout) => (
                            <div key={layout} className={`w-full flex h-1/2`}>
                                {baris.map(({icon, index}, wrapper) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            <button onClick={() => { alert("belum ada fiturnya")}}
                ...
                <div className="w-full border-y flex felx-col flex-1">
